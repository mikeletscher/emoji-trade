...
var API_URL = 'http://159.203.166.251:5555/';
  ...
  mounted: function() {
    ...
    this.$nextTick(function() {
      ...
      },
      {
        ...
        if ((['search']).indexOf(_vm.$route.name) < 0) {
          _vm.loading = false;
...
var EmojiViewPage = {
  ...
  created: function() {
    ...
    $.ajax({
      url: API_URL + 'charts?id=' + _vm.emojiId,
...
var LoginPage = {
  ...
  methods: {
    submitLogin: function(event) {
      ...
      var hash = loginForm.serializeArray().reduce(function(a, x) { a[x.name] = x.value; return a; }, {});;
...
var LogoutRoute = {
  template: "<div>You are being redirected</div>",

  created: function() {
    var _vm = this;

    Cookies.remove('et_token');

    _vm.$parent.isLoggedIn = false;
